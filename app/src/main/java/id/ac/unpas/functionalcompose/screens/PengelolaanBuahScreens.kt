...
@Composable
fun PengelolaanBuahScreen() {
    val context = LocalContext.current
    val db = Room.databaseBuilder(context, AppDatabase::class.java, "pengelolaan data Buah").build()
    val dataBuahDao = db.dataBuahDao()

    val list : LiveData<List<DataBuah>> = dataBuahDao.loadAll()
    val items : List<DataBuah> by list.observeAsState(initial = listOf())

    Column(modifier = Modifier.fillMaxWidth()) {
        FormPencatatanBuah(dataBuahDao)
